...
class BaseModel(nn.Module):
	...
	def __init__(self, args, corpus: BaseReader):
		...
		self.buffer = args.buffer  # 用于缓存开发集和测试集的输入数据,以提高效率
		self.optimizer = None
	...
	"""
	Auxiliary Methods -- 辅助方法
	"""
	def customize_parameters(self) -> list:
		# customize optimizer settings for different parameters
		"""
		自定义优化器参数，区分权重和偏置以应用不同的优化策略
		"""
		weight_p, bias_p = [], []
		# requires_grad 是一个布尔值属性，表示该参数是否需要计算梯度
		# 这里筛选出所有需要计算梯度的参数，让用户可以区分权重和偏置以应用不同的优化策略
		for name, p in filter(lambda x: x[1].requires_grad, self.named_parameters()):
			if 'bias' in name:
				bias_p.append(p)
			else:
				weight_p.append(p)
		# weight_decay是一种正则化技术,用于防止模型过拟合  -> 查看notion笔记、查看learning文件夹中的实例code
		# 对于权重参数,通常会应用weight_decay
		# 对于偏置参数,一般不使用weight_decay,所以这里设置为0
	...
	class Dataset(BaseDataset):
		...
		def __getitem__(self, index: int) -> dict:
			"""
			根据索引获取数据实例。如果模型启用了缓冲区且当前阶段不是训练阶段，
			则从缓冲区中返回对应索引的数据实例。否则，调用_get_feed_dict方法构建数据实例。
			"""
		...
		def prepare(self):
			"""
			在模型初始化后调用，用于准备数据。缓存数据是为了提高数据读取效率，减少重复计算。
			如果启用了缓冲区且当前阶段不是训练阶段，则缓存数据实例的输入数据。
			"""
			if self.model.buffer and self.phase != 'train':
		...
		def collate_batch(self, feed_dicts: List[dict]) -> dict:
			"""
			将多个数据实例的输入数据整理成一个批次。
			这段代码的作用是将多个数据实例的输入数据整理成一个批次。它遍历每个数据实例的键值对，
			并根据数据类型和长度的一致性，将它们转换为适当的格式（如numpy数组或pytorch张量），
			最终返回一个包含批次数据的字典。
			"""
			feed_dict = dict()
			for key in feed_dicts[0]:
				if isinstance(feed_dicts[0][key], np.ndarray):
					# item_id是np.ndarray类型
